...
func CheckMail(aim string) (string, int) {
	var mailConf MailboxConf
	mailConf.Title = "musicMod app邮箱验证"
	//这里就是我们发送的邮箱内容，但是也可以通过下面的html代码作为邮件内容
	// mailConf.Body = "坚持才是胜利，奥里给"
	mailConf.Body = "欢迎您注册musicMod app"
	...
	mailConf.SPassword = "ffvsstiwaicgebbj"
	...
	m.SetHeader(`From`, mailConf.Sender, "musicMod官方")
	...
	if err != nil {
		...
		return "", utils.ERROR_EMAIL_WRONG
	}
	log.Printf("Send Email Success")
	return vcode, utils.SUCCESS
}
